...
def run_programl(relative_path, input_file):
    graph = programl.from_clang([relative_path + input_file, "-O1 -emit-llvm -S -fno-discard-value-names -fno-inline-functions"])
    output = 'outputs/' + input_file + '_realPrograml'

    with open(output + '.dot', 'w') as f:
        f.write(programl.to_dot(graph))

    subprocess.run(["dot", "-Tpdf", output + ".dot", "-o",  output + ".pdf"], capture_output=True, text=True)
